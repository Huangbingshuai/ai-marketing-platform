...
- 产品字段和每个列表项均展示“用户事实”或“AI 图片建议”；人工修改后按用户事实处理，历史结果无法可靠判断时也不会伪造为 AI 建议。
- 制作规则只读继承资料导入节点。页面不提供时长、画幅、分辨率、渠道、禁用元素或视觉风格的第二套编辑入口，API 保存和校验时同样强制采用导入节点快照。
...
1. 页面结构与冻结原型一致，结果字段完整可见且逐项来源标签清晰。
