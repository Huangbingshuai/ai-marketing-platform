...
- 完整映射共享契约中的产品、卖点、用户、场景和制作配置字段，完成后可人工编辑业务事实并使用 `expectedRevision` 保存。
- 标准化节点对已结构化的融合结果执行确定性契约化：核心卖点最多 3 项，溢出项迁入次要卖点；次要卖点与人工编辑列表统一最多 20 项，用户事实不会因图片建议进入而被截断。只有确定性构造异常时才调用模型兜底。
...
1. 页面结构与冻结原型一致，结果字段完整可见且逐项来源标签清晰；图片建议在对应字段内显示来源图片名。
