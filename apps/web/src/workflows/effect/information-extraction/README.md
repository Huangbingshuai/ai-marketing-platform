...
- 标准化节点对已结构化的融合结果执行确定性契约化：卖点不分核心与次要，通常建议控制在 40 项以内，但不截断标准资料中的独立有效事实，也不限制用户继续添加；页面实时展示当前数量与软性精简建议。普通产品文档只保留与当前商品直接相关且可转化为可见产品素材的事实；结构化人工资料保持权威。没有可信卖点时明确提示补充资料，不使用占位卖点。
- 用户事实是信息卡的默认内容，页面不重复展示“用户事实”标签；人工修改后仍按用户事实处理，历史结果无法可靠判断时也不会伪造为 AI 补充。
- 同一源资料重提可保留真正的人工草稿；上游资料 revision 变化后以新资料卖点为准，旧卖点数组不再整组回写。修复前已混入 `generatedResult` 的冗余卖点覆盖会在下一次重提时清除。运行中的旧保存请求会被取消，返回结果还需匹配当前 `resultId + revision` 才能更新页面。
- 图片模型的安全补充经逐条审查后直接进入信息卡；即使现有卖点已达到建议数量，独立有效的可见事实仍会保留。仅对这些项在输入框下方显示无底色的“图片识别补充 · 来源文件名”辅助说明；不占用左侧字段名称空间，也不再额外展示大块建议列表。
