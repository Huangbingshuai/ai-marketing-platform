...
公开子工作流先准备已确认的产品事实，再通过一次批次级 AI 调用编译事实视觉角色：哪些事实可以直接拍、哪些适合动作展示、哪些只能作商业背景、哪些禁止用画面证明。每次用户重新批量生成都会重新编译事实视觉策略，不从历史批次复用；同一任务因网络或 Worker 故障重试时才允许恢复本 Run 已成功的检查点。随后共用提示词只编译一次，批次创意导演会结合事实视觉策略与信息卡中的视觉风格基调规划差异化方向；Worker 会把模型遗漏的必用事实补充到相容且负载较低的方向，避免整批只围绕产品名称、规格和外观。风格基调只控制光线、色彩、材质和镜头质感，不能反向虚构厨房、门店、节庆或人物场景。每条创意任务仍只领取一个可见画面任务、一个主要业务事实、产品锚点和一个创意方向，再由模型同时生成干净正文与六维创意信息。独立评估会核对证据摘录是否真实存在且与事实语义一致；包装规格不能证明配方，普通场景不能证明工艺，错误绑定会被删除。
