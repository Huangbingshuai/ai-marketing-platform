...
| 03 Prompt 生成 | 已完成 | 生成连贯六维素材 Prompt 批次；先编译事实视觉角色和批次创意方向，再生成、独立评估并用正文向量与业务语义簇联合 MMR 精确择优，完成校验后提交 `prompt-batch:{productId}`。 |
...
公开子工作流先准备已确认的产品事实，再通过一次批次级 AI 调用编译事实视觉角色：哪些事实可以直接拍、哪些适合动作展示、哪些只能作商业背景、哪些禁止用画面证明。同一份提炼内容会按内容哈希复用策略，不随 50 条 Prompt 重复调用。随后共用提示词只编译一次，批次创意导演会结合事实视觉策略与信息卡中的视觉风格基调，规划 8 个彼此有业务意义差异的创意方向；风格基调只控制光线、色彩、材质和镜头质感，不能反向虚构厨房、门店、节庆或人物场景。每条创意任务只领取一个可见画面任务、少量商业背景、产品锚点和一个创意方向，再由模型同时生成干净正文与六维创意信息：叙事结构、场景、人物、产品关联点、镜头语言和情绪基调。产品名称等锚点负责让画面能认出当前商品；已确认规格、包装和外观单独作为产品边界，只阻止补造，不要求每条正文机械复述。六维必须围绕同一个创意一起产生，禁止从独立词池随机拼装。独立评估环节复用同一视觉策略检查产品相关性、事实表达、创意连贯性、画面可执行性和商业素材价值，并进行多用途分类；它只评价，不改写正文。

目标 50 条时，Worker 首轮按目标数量的 `140%` 生成 70 条候选，再按质量与差异化择优保留 50 条。硬性淘汰只覆盖与当前产品无关、使用未确认商业事实、六维与正文严重冲突、内容破损或不可执行、多场景完整成片结构和实质完全重复；一般相似、用途分布不均或次要事实暂未使用只记录提醒。合格数量不足时只执行一次定向补充，候选总量不超过目标数量的 `160%`；成功结果必须严格等于用户设置的总数量，不再要求六类配额。
