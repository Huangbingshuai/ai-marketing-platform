...
只有数量、六类配额、提炼核心事实覆盖、片段职责、执行有效性、至少三维差异和双重重复度全部通过后，页面才允许“完成校验”并提交 `prompt-batch:{productId}` WorkingArtifact。详细设计、V1～V4 兼容和验收记录见 [差异化 Prompt 批量生成节点实施方案](docs/效果类工作流-差异化Prompt批量生成节点实施方案.md)。
