...
| 03 Prompt 生成 | 当前视觉策略与向量择优流程已完成 | 生成连贯六维素材 Prompt 批次；先编译事实视觉角色，再生成、独立评估并用正文向量与业务语义簇精确择优，完成校验后提交 `prompt-batch:{productId}`。 |
...
- 当前 Prompt 批次严格匹配用户设置的总数量；六类是生成后的推荐/兼容用途标签，不再是生成配额或六条独立生产线。
...
2026-08-31 基于当前工作区代码完成全量自动回归：

| 范围           | 结果                             |
| -------------- | -------------------------------- |
| 共享 Contracts | 3 个测试文件、17 项测试通过      |
| NestJS API     | 31 个测试文件、300 项测试通过    |
| Vue Web        | 29 个测试文件、161 项测试通过    |
...
5. 制作规则：统一时长、画幅、分辨率、投放渠道、全局禁用元素、视觉风格基线。分辨率只能选择 `480p / 720p / 1080p`。
...
当前页面只开放两个批次设置：Prompt 总数量和统一默认片段时长。默认是 50 条、每条 5 秒，当前实现支持在 4～30 秒内选择，已有商品草稿继续保留用户保存的时长；不再设置六类数量、六类时长、语义重复率或画面重合率。钩子、痛点、产品展示、卖点讲解、结尾转化和片尾品牌是生成后的素材用途标签，一条 Prompt 可以有一个推荐用途和多个兼容用途。

公开子工作流先准备已确认的产品事实，再通过一次批次级 AI 调用编译事实视觉角色：哪些事实可以直接拍、哪些适合动作展示、哪些只能作商业背景、哪些禁止用画面证明。同一份提炼内容会按内容哈希复用策略，不随 50 条 Prompt 重复调用。随后共用提示词只编译一次，每条创意任务只领取一个可见画面任务、少量商业背景和产品锚点，再由模型同时生成干净正文与六维创意信息：叙事结构、场景、人物、产品关联点、镜头语言和情绪基调。产品名称等锚点负责让画面能认出当前商品；已确认规格、包装和外观单独作为产品边界，只阻止补造，不要求每条正文机械复述。六维必须围绕同一个创意一起产生，禁止从独立词池随机拼装。独立评估环节复用同一视觉策略检查产品相关性、事实表达、创意连贯性、画面可执行性和商业素材价值，并进行多用途分类；它只评价，不改写正文。

批量生成会先协调 8～12 个创意方向，再把方向和事实均衡分给候选；50 条目标默认先生成 70 条。最终仍按 `70%` 单条质量和 `30%` 相对新颖度择优，其中新颖度同时考虑干净正文向量与叙事、场景、人物关系、产品动作、镜头、情绪六轴业务簇。场景或动作过度集中只会触发一次最多 10 条的定向补充和软降权，不会因机械配额阻断 50 条精确数量。
...
精确择优采用单正文向量 MMR：先清理共用尾段、技术参数和共同产品词，再通过火山向量接口生成正文向量；Worker 用 NumPy `float32` 矩阵一次计算余弦相似度，按 `70%` 独立质量分和 `30%` 相对已选集合的新颖度逐条选择。人工保留项和单条重生成时的其他 Prompt 作为固定参照。高风险近重复只会触发一次受控候选补充或软告警，一般相似不会被硬删除，也不会阻止达到准确数量。
...
时长只作为结构化参数和轻量创作上下文：Worker 将 4～8、9～15、16～22、23～30 秒分别编译成“快速聚焦、完整动作、渐进过程、连贯阶段”的软节奏意图。它不规定固定动作数、镜头数或字数，也不写进正文。评估器用现有“画面可执行性”评分检查动作密度，轻微过密或过稀只记录软提醒，不影响精确数量补齐。

当前 Prompt 工作流只展示并执行八阶段线性拓扑，其中“事实视觉使用策略编译”位于“提炼信息应用映射”之后、“共用提示词编译”之前。数据库中的旧 Run、Stage 和 Shard 只作为不可见审计记录保留，不再进入工作区、节点详情或 Worker；已经生成的 Prompt 结果不会因此删除。详细设计见 [单一当前工作流收敛实施方案](docs/workflows/effect/plans/效果类Prompt-单一当前工作流收敛实施方案.md)，质量改造见 [Prompt 质量提升分步实施总方案](docs/workflows/effect/plans/效果类Prompt-V11质量提升分步实施总方案.md)，通俗说明见 [Prompt 生成子工作流节点通俗说明](docs/workflows/effect/guides/效果类Prompt生成子工作流节点通俗说明.md)。
...
完整 Prompt 批次必须严格满足用户设置的总数量，并且不存在硬性质量问题；当前工作流不要求六类配额，一般语义相似也不会阻断数量。失败任务不会覆盖上一份有效结果。若失败前已有分片成功，结果区会展示其中通过基础结构检查的 Prompt 作为“临时预览”；临时预览只能查看、搜索、分页和复制，不能编辑、导出或完成校验。重新批量生成成功后才会形成新的正式节点结果。
