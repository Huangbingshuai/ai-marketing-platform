...
- 火山正文向量 + OpenMontage 式 MMR 择优：质量占 70%、相对新颖度占 30%，人工保留项和单条重生成的其他条目作为固定参照；相似度达到 82% 的连通组按 `n-1` 计算重复条目，整批语义重复度必须严格低于 15%。
...
| NestJS API     | 31 个测试文件、297 项测试通过    |
| Vue Web        | 29 个测试文件、160 项测试通过    |
| 提炼 Worker    | 81 项通过、4 项付费/集成门控跳过 |
| Prompt Worker  | 168 项通过、1 项付费集成门控跳过 |
...
精确择优采用单正文向量 MMR：先清理共用尾段、技术参数和共同产品词，再通过火山向量接口生成正文向量；Worker 用 NumPy `float32` 矩阵一次计算余弦相似度，按 `70%` 独立质量分和 `30%` 相对已选集合的新颖度逐条选择。人工保留项和单条重生成时的其他 Prompt 作为固定参照。相似度达到 `82%` 的正文会形成连通重复组，每组按 `n-1` 计算重复条目；整批重复度必须严格 `<15%`。50 条最多允许 7 条重复，8 条即进入 `NEEDS_REVIEW`。达到上限会触发一次受控多样性补充，不会用规则模板拼接凑数。
