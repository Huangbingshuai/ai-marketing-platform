...
当前优先建设效果类黄金链路：资料导入、AI 信息提炼、片段级 Prompt 生成和视频渲染节点已完成；模板混剪具备正式工作台、异步智能填充与草稿/提交边界，成片输出仍待开发。
...
| 04 视频渲染    | 已完成 | 正式页面接入真实素材工作区；按已确认 Prompt 创建异步批次，支持轮询、预览、单条重生成、返修候选、导入/删除/导出，并在完成校验后提交素材工作副本。 |
...
- Seedance 素材片段异步渲染、租约恢复与候选返修；正式渲染页消费真实 API 批次与素材内容，Mock Service 仅保留给测试。
...
- “产品基础 + 统一卖点”信息卡、人工修正、来源追踪、冲突报告和历史字段读取折叠。
...
事实字段的融合优先级为：当前人工修正 > 文档 > 电商 > 图片。当前信息卡只保留产品品类、名称、核心规格、价格带、核心外观特征和统一卖点列表。口味、原料、工艺、结构、性能、吃法、使用方式、适用人群、用户需求、使用场景和可信背书，只要有资料依据、与当前商品直接相关且能自然转化为可见产品素材，都可以作为独立卖点事实保存，不再拆成互相重复的词池。文档 AI 会排除竞品、泛行业常识、纯文字购买建议、保存或安全提醒及未经商品证据支持的健康功效；图片模型只能补充可见外观、包装文字、明确动作和明确场景，不能推断配方、工艺、产地、认证、功效、销量或消费者评价。卖点通常建议控制在 40 项以内，但这是质量建议而非硬上限；用户可继续添加，结构化标准资料包中超过 40 项的独立有效事实也会完整保留。页面实时展示当前数量与精简建议；没有可信卖点时明确提示补充资料，不生成占位卖点。每项图片建议直接显示在对应卖点下并标注来源图片。结构化融合结果由 Worker 确定性契约化并通过 Pydantic 校验，只有异常时才调用标准化模型兜底。
...
2. 统一卖点：每条是一项可独立理解、有资料依据并可供视频创意使用的产品事实。

历史信息卡中的核心/次要卖点、可信背书、受众、痛点、决策动因和三类场景在 API 读取边界按原顺序折叠到统一卖点，不调用模型、不批量改写数据库，也不因此增加 revision。营销目标和历史视频制作配置不会迁入卖点。

生成成功和编辑防抖只保存领域结果与页面草稿；用户点击“完成校验”后才提交 `marketing-insight:{productId}` 工作副本。历史结果通过同一个读取归一化入口折叠，不要求一次性回填历史数据。
...
当前页面开放 Prompt 总数量、统一片段时长、投放渠道、视觉风格基调和禁用元素。视觉风格默认“AI 根据场景选择”，也可指定整批基调；不再按用途设置数量、时长、语义重复率或画面重合率。钩子、产品展示、效果和结尾转化是生成后的素材用途标签，一条 Prompt 可以有一个推荐用途和多个兼容用途；效果片段只表达已确认的使用结果或产品价值，不允许用画面虚构配方、工艺或功效证明。新生成 Prompt 只描述可混剪的无口播广告素材：禁止人物口播、旁白、逐字台词、歌词、字幕文案和可辨识说话口型，静音时仍需看懂产品动作与素材价值；只保留自然环境声和产品动作 Foley，配乐、统一口播及字幕交给后续混剪/成片。

素材 Prompt 按可执行的时间顺序描述：从必要初始状态或主动作直接开始，明确主体与商品的空间关系、一个连续产品动作、镜头每次变化获得的新信息和结束画面。生成仍为一次结构化模型调用，不增加第二次润色调用；Worker 只把模型生成的 `shotPlan` 分配时间并排版，不补写创意或营销语义。

视觉风格和渠道直接来自 Prompt 批次设置，不再回读信息卡的历史视频配置。画幅、分辨率和 Seedance 能力由视频渲染节点按商品独立保存；修改它们只影响之后创建的渲染任务，不会使信息提炼或 Prompt 过期。

批量生成由 AI 根据已确认卖点直接安排精确数量的素材任务，不再调用空间、方向、路线规划及重复关系复核。默认一条独立可呈现卖点对应一条素材；任务少时自然组合，多时设计不同观看事件。规划按容量分页，完整事实和已有任务持续作为上下文；Worker 不做语义分配。初始生成扣除人工保留项后的目标数，数量/覆盖/近似问题最多合并一次补充；补充不超过目标数的 20% 或真实数量缺口（取较大值）。独立 AI 读取全部确认事实和视觉边界，判断实际实现程度，允许绑定未预分配但实际使用的合法事实。Worker 只检查来源、ID、数量、结构、枚举、revision 和哈希，不做语义、关键词或字符相似判断。商品参考图经快照、租约、大小与 SHA-256 核验及压缩后用于事实视觉策略和 Turbo 候选生成，不进入评分请求；仅帮助认识外形与操作，不增加配方、功效或材质事实。`PROMPT_CANDIDATE_PRODUCT_IMAGES` 默认 true，仅用于受控文字/图片对照。新 Run 不复用旧计划，只有同 Run 恢复有效检查点。

目标 50 条时首轮生成 50 个素材任务，不再预生成 70 条。卖点安排、实际实现和上下文保留分开统计；正常候选不再全部经过前置改写，仅在独立评估发现明确问题后定向修订并复评。

择优继续使用正文向量与六轴语义簇、余弦相似度和 MMR。82% 相似关系形成连通重复组，15% 重复度仍是软提醒，不因一般相似阻止满足数量与安全条件的用户确认。补充直接重设计具体素材任务，不再规划补充空间与方向。
...
- 后一次生成失败不会删除或遮蔽此前成功结果，也不会阻止最新的已完成、数量与安全结构满足要求的结果执行“完成校验”；一般质量建议不锁定用户提交。
...
事实视觉策略、连贯创意与独立评估均通过小分片执行；Prompt 片段时长统一为 4～15 秒，与当前 Seedance 2.0 能力上限一致。候选生成会结合时长和实际输出 Token 上限动态装箱，异常长正文在评估时自动单条执行。多条结构化响应被截断或格式异常时，Worker 只把当前逻辑分片递归拆小，已经成功的分片仍可在同一 Run 中恢复；用户主动重新批量生成则创建新 Run 并从头执行。策略阶段默认超时 180 秒，候选阶段默认超时 120 秒；运行失败不会自动切换模型。精确数量阶段使用火山正文向量和本地 NumPy MMR，不逐对远程调用。
...
Docling 解析与 Ark 文档卖点抽取是两个阶段。产品信息表的 Markdown 表格，以及 Word 经 Docling 转换后的“字段标题 + 段落/项目列表”，都会直接按字段白名单解析，不调用 Ark。普通产品介绍、评测文章、说明书或品牌 Brief 会按标题和自然段切成有限分片，再由文档专用强模型提炼卖点；该提示表示其中的模型请求超时，界面只公开安全化的错误类型、尝试次数和耗时，不展示正文或模型请求。默认单片超时为 180 秒、输出上限 8192 Token、最多并发 2 个分片；文档事实抽取使用最小思考强度，把预算优先留给完整结构化结果。
...
不需要。只需在本机 `.env` 填写 `ARK_API_KEY`。普通产品文档默认使用 Seed 2.1 Pro，其余常规模型调用默认使用 Seed 2.1 Turbo；`ARK_MODEL`、`ARK_DOCUMENT_MODEL`、`ARK_IMAGE_MODEL` 和 `ARK_NORMALIZATION_MODEL` 都可以按部署需要覆盖。
...
- [Prompt 视觉事件修复付费续测报告](docs/workflows/effect/plans/效果类Prompt-视觉事件修复付费续测报告.md)
...
- 视频渲染节点已完成正式前后端与 Seedance Worker 链路：真实批次、预览、单条重生成、参考视频返修、素材导入/删除/导出及人工完成校验均有对应接口和页面交互。节点功能完成不意味着所有生成视频的画面质量都已达标，仍需按商品和批次审片。模板混剪已接入正式工作台和异步智能填充，仍须通过同 Run 真实素材池、浏览器宽窄屏和 Worker 端到端验收；成片输出尚未进入当前实现范围。
