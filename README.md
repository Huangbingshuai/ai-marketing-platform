...
公开子工作流先准备已确认的产品事实，再通过一次批次级 AI 调用编译事实视觉角色：哪些事实可以直接拍、哪些适合动作展示、哪些只能作商业背景、哪些禁止用画面证明。同一份提炼内容会按内容哈希复用策略，不随 50 条 Prompt 重复调用。随后共用提示词只编译一次，批次创意导演会结合事实视觉策略与信息卡中的视觉风格基调，规划 8 个彼此有业务意义差异的创意方向；Worker 会把模型遗漏的必用事实补充到相容且负载较低的方向，避免整批只围绕产品名称、规格和外观。风格基调只控制光线、色彩、材质和镜头质感，不能反向虚构厨房、门店、节庆或人物场景。每条创意任务仍只领取一个可见画面任务、少量商业背景、产品锚点和一个创意方向，再由模型同时生成干净正文与六维创意信息。独立评估环节同时核对直接画面事实与已分配业务背景；后者只有在正文真实体现人物、场景、需求、动机或消费语境时才进入 `CONTEXT` 绑定，不要求机械复述信息卡原词，也不得冒充配方、工艺或功效的视觉证明。

目标 50 条时，Worker 首轮按目标数量的 `140%` 生成 70 条候选，再按质量与差异化择优保留 50 条。质量与新颖度权重保持不变；当仍有必用事实未覆盖且存在合格候选时，先在能补齐缺口的候选中按同一公式择优。最终结果分别记录画面直接依据和创意背景依据，并输出必用、已覆盖、缺失和自适应暂未使用事实；仍有必用事实缺失时结果进入 `NEEDS_REVIEW`，但不会伪造绑定或用短批次绕开问题。
