...
事实字段的融合优先级为：当前人工修正/表单 > 文档 > 电商 > 图片。图片模型可根据明确画面证据保守补充卖点、痛点、受众、决策动因、营销目标和场景建议，但不能覆盖已经确认的用户事实；核心外观特征只在资料缺失时由识图补齐。图片卖点在保留用户清单后按“产品可见价值 + 场景建议”择取补充，节庆、礼赠、家庭餐桌等证据优先归入对应场景。每项图片建议直接显示在对应信息卡字段内，并标注来源图片。数组字段采用稳定去重，同级冲突保留安全告警；结构化融合结果由 Worker 确定性契约化并通过 Pydantic 校验，只有异常时才调用标准化模型兜底。
