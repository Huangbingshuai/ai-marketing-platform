...
事实字段的融合优先级为：当前人工修正/表单 > 文档 > 电商 > 图片。当前信息卡只保留产品品类、名称、核心规格、价格带、核心外观特征和统一卖点列表。口味、原料、工艺、结构、性能、吃法、使用方式、适用人群、用户需求、使用场景和可信背书，只要有资料依据且能帮助后续视频创意，都作为独立卖点事实保存，不再拆成互相重复的词池。图片模型只能补充可见外观、包装文字、明确动作和明确场景，不能推断配方、工艺、产地、认证、功效、销量或消费者评价。卖点最多 100 项，只清理空值和完全相同的重复值；没有可信卖点时明确提示补充资料，不生成占位卖点。每项图片建议直接显示在对应卖点下并标注来源图片。结构化融合结果由 Worker 确定性契约化并通过 Pydantic 校验，只有异常时才调用标准化模型兜底。
