...
公开子工作流先准备已确认的产品事实，再通过一次批次级 AI 调用编译事实视觉策略：哪些事实可以直接拍、哪些适合动作展示、哪些只能作商业背景、哪些禁止用画面证明。每次用户重新批量生成都会重新编译，不从历史批次复用；同一任务因网络或 Worker 故障重试时才允许恢复本 Run 已成功的检查点。随后共用提示词只编译一次，批次创意导演会结合事实视觉策略与信息卡中的视觉风格基调规划差异化方向；富信息商品的每个方向由模型关联 2～4 项卖点、痛点、受众、决策动机、营销目标或场景，并逐项说明这些事实如何共同服务当前创意。整批方向必须覆盖全部已确认业务事实；首次遗漏时由模型根据缺失清单完整重规划，Worker 不再随机补挂事实。风格基调只控制光线、色彩、材质和镜头质感，不能反向虚构厨房、门店、节庆或人物场景。每条创意任务完整继承所属方向的事实使用方案和只负责商品真实性边界的 `productSnapshot`，再由模型同时生成干净正文与六维创意信息。生成模型返回每项计划事实的逐字证据位置，Worker 只校验引用和位置；独立评估模型再逐项判断事实语义是否完整。
