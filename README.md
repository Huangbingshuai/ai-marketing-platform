...
| 03 Prompt 生成 | V11 已完成 | 生成 V6 连贯六维素材 Prompt 批次；只有数量和硬性质量条件通过并完成校验后提交 `prompt-batch:{productId}`。 |
...
公开子工作流先准备已确认的产品事实和一段批次级共用提示词，再让模型同时生成干净正文与六维创意信息：叙事结构、场景、人物、产品关联点、镜头语言和情绪基调。六维必须围绕同一个创意一起产生，禁止从独立词池随机拼装。独立评估环节随后检查产品相关性、创意连贯性、画面可执行性和商业素材价值，并进行多用途分类；它只评价，不改写正文。
...
V11 新任务按七阶段线性拓扑展示；V8、V9、V10 历史 Run 仍按原拓扑只读查看。V5 历史结果保持原始存储，前端只读适配原片段类型和“卖点侧重”，不会伪造 V11 多用途评估。详细设计和兼容口径见 [差异化 Prompt 批量生成节点实施方案](docs/workflows/effect/plans/效果类工作流-差异化Prompt批量生成节点实施方案.md)，通俗说明见 [Prompt 生成子工作流节点通俗说明](docs/workflows/effect/guides/效果类Prompt生成子工作流节点通俗说明.md)。
