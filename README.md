...
| 02 AI 信息提炼 | 已完成 | 生成《产品素材制作信息卡》；只有完成校验后提交 `marketing-insight:{productId}`。                              |
| 03 Prompt 生成 | V11 视觉策略版已完成 | 生成 V6 连贯六维素材 Prompt 批次；先编译事实的视觉角色，再生成干净正文，只有数量和硬性质量条件通过并完成校验后提交 `prompt-batch:{productId}`。 |
...
2026-08-27 基于当前工作区代码完成全量自动回归：

| 范围           | 结果                          |
| -------------- | ----------------------------- |
| 共享 Contracts | 3 个测试文件、14 项测试通过   |
| NestJS API     | 31 个测试文件、268 项测试通过 |
| Vue Web        | 28 个测试文件、159 项测试通过 |
| Prompt Worker  | 116 项 pytest 通过            |

Contracts、API 和 Web 的 TypeScript 类型检查与生产构建均已通过；Prompt 页面已完成桌面和 720px 窄屏浏览器验收。自动测试默认不调用 Ark 或 Seedance 付费接口。
...
NestJS API ───── Prisma ───── PostgreSQL
   ...
   └──────────── Python LangGraph Workers
                         ...
                         └─ Prompt 生成：Seed 2.1 Turbo 连贯创意 + 独立评估
```
...
V11 页面只开放两个批次设置：Prompt 总数量和统一默认片段时长。默认仍为 50 条、每条 5 秒；不再设置六类数量、六类时长、语义重复率或画面重合率。钩子、痛点、产品展示、卖点讲解、结尾转化和片尾品牌改为生成后的素材用途标签，一条 Prompt 可以有一个推荐用途和多个兼容用途。

公开子工作流先准备已确认的产品事实，再通过一次批次级 AI 调用编译事实视觉角色：哪些事实可以直接拍、哪些适合动作展示、哪些只能作商业背景、哪些禁止用画面证明。同一份提炼内容会按内容哈希复用策略，不随 50 条 Prompt 重复调用。随后共用提示词只编译一次，每条创意任务只领取一个可见画面任务、少量商业背景和产品锚点，再由模型同时生成干净正文与六维创意信息：叙事结构、场景、人物、产品关联点、镜头语言和情绪基调。六维必须围绕同一个创意一起产生，禁止从独立词池随机拼装。独立评估环节复用同一视觉策略检查产品相关性、事实表达、创意连贯性、画面可执行性和商业素材价值，并进行多用途分类；它只评价，不改写正文。

目标 50 条时，Worker 首轮生成约 60 条候选，再按质量与差异化择优保留 50 条。硬性淘汰只覆盖与当前产品无关、使用未确认商业事实、六维与正文严重冲突、内容破损或不可执行、多场景完整成片结构和实质完全重复；一般相似、用途分布不均或次要事实暂未使用只记录提醒。合格数量不足时只执行一次定向补充，成功结果必须严格等于用户设置的总数量，不再要求六类配额。
...
V11 新任务按八阶段线性拓扑展示，其中“事实视觉使用策略编译”位于“提炼信息应用映射”之后、“共用提示词编译”之前；V8、V9、V10 和原 V11 历史 Run 仍按原拓扑只读查看。V5 历史结果保持原始存储，前端只读适配原片段类型和“卖点侧重”，不会伪造 V11 多用途评估或视觉策略。详细设计和兼容口径见 [差异化 Prompt 批量生成节点实施方案](docs/workflows/effect/plans/效果类工作流-差异化Prompt批量生成节点实施方案.md)，本次分步改造见 [Prompt V11 质量提升分步实施总方案](docs/workflows/effect/plans/效果类Prompt-V11质量提升分步实施总方案.md)，通俗说明见 [Prompt 生成子工作流节点通俗说明](docs/workflows/effect/guides/效果类Prompt生成子工作流节点通俗说明.md)。
...
- 共用提示词编辑的是批次最终完整文本，保存时使用结果 revision 做并发校验并恢复为待校验状态；再次完成校验前不会覆盖已经提交的工作副本。
...
策略规划使用低成本模型生成精简事实关系骨架，候选分支使用 Turbo；每片最多 8 条、全图最大并发 3，并按小分片动态限制输出 Token。策略规划默认超时 180 秒，候选生成默认超时 120 秒，Provider 单次调用默认只尝试 1 次；运行失败不会自动切换模型。
...
核心接口包括加载产品工作区、保存总数量与统一时长、按推荐/兼容用途分页筛选、启动批量生成、单条重生成或 `ITEM_EVALUATE`、查询 Run 与安全节点详情、人工增删改、保存批次共用提示词、完成校验和权威 JSON 导出。共用提示词编辑只更新领域草稿并使用 revision 并发控制，再次完成校验后才更新 `prompt-batch:{productId}` WorkingArtifact。Worker 内部 API 使用独立 `EFFECT_PROMPT_WORKER_TOKEN` 与 attempt token。
...
先检查 `effect-prompt-generation-worker` 日志中的 claim 状态。开发环境的 API 与 Compose 已统一使用本地 Worker Token 默认值；生产环境仍必须显式配置 `EFFECT_PROMPT_WORKER_TOKEN`。V11 将创意生成和独立评估拆成小分片；若 Ark 明确返回输出上限截断，任务会以 `AI_OUTPUT_TRUNCATED` 停止，不会把同一个不可完成请求盲目重复三次。

### Prompt 任务失败后页面没有任何提示词

完整 Prompt 批次必须严格满足用户设置的总数量，并且不存在硬性质量问题；V11 不再要求六类配额。失败任务不会覆盖上一份有效结果。若失败前已有分片成功，结果区会展示其中通过基础结构检查的 Prompt 作为“临时预览”；临时预览只能查看、搜索、分页和复制，不能编辑、导出或完成校验。重新批量生成成功后才会形成新的正式节点结果。

工作流中的红色节点应只有真实失败分支；同批次其他被中断分支显示“已跳过”，不会继续显示“执行中”。若旧失败记录曾保存了错误的当前节点，公开接口会优先按真实失败 Stage 修正显示。

### Prompt 长时间停在“连贯六维创意生成”或“创意质量评估”

V11 新任务不再执行 V8～V10 的营销关系、六类坐标和六类 Prompt 分支，而是按小分片执行连贯创意生成与独立评估。页面中的更新时间会随 Worker 心跳刷新并显示尝试次数；刷新页面后继续从后端恢复状态。历史 V8、V9、V10 Run 仍按原拓扑只读展示，因此打开旧任务时仍可能看到旧节点名称。
