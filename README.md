...
| 03 Prompt 生成 | 已完成 | 生成 V5 素材片段 Prompt 批次；只有数量、质量和覆盖门禁通过并完成校验后提交 `prompt-batch:{productId}`。       |
...
- 独立 Prompt 生成 Worker：全局事实分配、六类 AI 营销规划并行分支、素材片段条件路由、执行门禁、双重去重和最多三轮定向补齐。
- Prompt V5 批次严格匹配用户设置的总数量和六类配额，使用稳定提炼事实绑定、批次级渲染参数和可扩展共用提示词。
...
| 范围           | 结果                          |
| -------------- | ----------------------------- |
| 共享 Contracts | 3 个测试文件、23 项测试通过   |
| NestJS API     | 31 个测试文件、232 项测试通过 |
| Vue Web        | 27 个测试文件、148 项测试通过 |
| Prompt Worker  | 84 项 pytest 通过             |
...
公开子工作流先把已提交信息卡映射为稳定 `factId`，再由 Worker 将强制事实分配到职责兼容的六类片段。钩子、痛点、产品展示、卖点讲解、结尾转化和片尾品牌分别调用独立 AI 营销规划分支；每支只为本类生成少量完整创意母版，场景、人物、动作、镜头、情绪和结束状态始终作为一个整体进入蓝图，不再拆成独立词池随机组合。候选生成继续使用 Doubao Seed 2.1 Turbo；事实来源、安全证据、职责、执行、去重、覆盖和数量门禁仍由 Worker 确定性校验。

V9 六类规划分支默认并行，单支超时 120 秒、输出上限 3072 Token、Provider 单次调用一次。成功分支保存经过验证的检查点，任务级重试只重新调用失败分支；网络、超时、429 和 5xx最多受三次业务尝试约束，完整响应的 JSON 格式异常独立获得一次额外重试机会，明确输出截断立即停止。Worker 失联后由 API 自动回收过期租约并重新排队，页面按 Run 的 `graphVersion` 展示 V8 历史拓扑或 V9 六分支拓扑。
...
先检查 `effect-prompt-generation-worker` 日志中的 claim 状态。开发环境的 API 与 Compose 已统一使用本地 Worker Token 默认值；生产环境仍必须显式配置 `EFFECT_PROMPT_WORKER_TOKEN`。新版规划只返回精简事实关系骨架；若 Ark 明确返回输出上限截断，任务会以 `AI_OUTPUT_TRUNCATED` 立即停止，而不是重复三次。

### Prompt 任务失败后页面没有任何提示词

完整 Prompt 批次仍必须严格满足用户设置的总数和六类配额，失败任务不会保存短批次，也不会覆盖上一份有效结果。若失败前已有分片成功，结果区会展示其中通过基础结构检查的 Prompt 作为“临时预览”；临时预览只能查看、搜索、分页和复制，不能编辑、导出或完成校验。重新批量生成成功后才会形成新的正式节点结果。

工作流中的红色节点应只有真实失败分支；同批次其他被中断分支显示“已跳过”，不会继续显示“执行中”。若旧失败记录曾保存了错误的当前节点，公开接口会优先按真实失败 Stage 修正显示。

### Prompt 长时间停在“营销关系规划”

新任务使用 V9 六类并行规划，不再执行一个超长“营销关系规划”请求。2026-08-27 的“广式腊肠”真实验收中，六支分别耗时 18.7～38.4 秒、输出 851～1718 Token，全部在一次尝试内完成并进入片段蓝图编排；每支上限为 120 秒和 3072 Token。页面中的更新时间会随 Worker 心跳刷新并显示尝试次数；单支失败时其余成功检查点会保留，只重做失败分支。历史 V8 任务仍按旧拓扑展示。
