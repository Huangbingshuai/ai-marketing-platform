...
> 当前实现口径（2026-09-01）：本节点已由下文记录的历史“同字段 Mini 归并”升级为一次 Turbo 严格结构化决策。模型负责跨字段归类、同义/父子/同主题关系和字段超量择优；Worker 只校验事实 ID、来源权限、字段数量和结构，不再承担任何字符或业务语义判断。历史章节保留用于解释演进过程，不代表当前运行架构。
