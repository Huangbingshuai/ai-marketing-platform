...
## 32. 2026-08-28 受众事实与分辨率下游贯通

此前“只展示五项全局视频配置且不展示分辨率”的口径已被本节覆盖。资料导入节点新增可见分辨率后，信息提炼继续从不可变 `globalVideoConfig` 快照读取并写入产品素材制作信息卡。Prompt 的提炼信息应用映射必须逐项读取规范 `targetAudiences[]`，每个受众形成独立事实；只有历史结果缺少该数组时才回退到 `targetAudience` 兼容摘要。同时把 `resolution` 登记为 `RESOLUTION` 全局约束，供批次 `renderProfile` 和后续视频请求使用，不写入单条 Prompt 正文。

产品素材制作信息卡的“制作规则层”同步提供不可创建自定义值的分辨率下拉框，只允许 `480p / 720p / 1080p`。该字段与时长、画幅等规则共用现有自动保存和 WorkingArtifact 提交边界；绕过前端提交的自定义分辨率由 API 拒绝，历史大写合法值读取及保存时规范为火山接口使用的小写形式。FORM 节点详情同时展示本次冻结的真实分辨率。
