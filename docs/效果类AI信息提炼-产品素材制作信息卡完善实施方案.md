...
- 核心卖点 1～3 项；超出部分按含义迁入次要卖点，不允许截断丢失。
- 次要卖点、信任背书最多 6 项；用户痛点、决策动因和三类场景最多 5 项。
- 信任背书只能来自明确证据；没有证据时为空数组。
- 时长、画幅、渠道和视觉风格的初始值继承资料导入节点全局配置；用户可在信息卡中通过下拉框形成字段级人工覆盖。禁用词继续至少包含资料导入节点的全局禁用词。
...
- 场景层展示使用、购买和情绪共鸣场景。
- 制作规则层使用与资料导入节点一致的选项展示时长、画幅、渠道和视觉风格基线，初始继承后允许人工下拉调整；禁用词保持可编辑标签形式。
...
- 前端已按五层信息结构展示并编辑全部 V2 字段，核心卖点限制为 1～3 项；未恢复来源提示横幅。
