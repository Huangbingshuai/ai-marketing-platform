...
- `ITEM_EVALUATE` 只重新评估用户新增或修改后的原正文，不生成替代正文；评估时读取当前全部可信事实，并从正文推导创意主线、六维信息、事实绑定、推荐用途和兼容用途。API 在完成时按稳定 ID 合并回原批次；无硬问题为 `VERIFIED`，事实编造、产品无关或正文破损等硬问题为 `NEEDS_REVISION`，且保留安全问题代码供页面提示。
