...
新任务只使用 `CURRENT_EFFECT_PROMPT_GRAPH_VERSION` 指向的当前八节点拓扑。Worker 先把已提交营销洞察中的可用事实交给一次独立 AI 调用，编译为“产品身份锚点、可直接呈现、可通过动作呈现、仅作商业背景、仅适合文字表达、禁止视觉证明”六种角色；Worker 再校验事实引用、完整覆盖和禁止推导，并按营销洞察 `contentHash + 策略模板版本` 复用成功检查点。策略不会写入公开 Prompt 结果，也不会按候选数量重复调用。
...
当前视觉策略模板为 `effect-prompt-v11-fact-visual-strategy-v2`，连贯创意模板为 `effect-prompt-v11-coherent-creative-v13`。生成角色保持厂商无关，一次调用直接产出可供后续渲染的完整素材 Prompt；镜头、光线和情绪必须服务本条产品动作，不得给不同候选机械套用统一的“电影级、商业广告、高级质感、暖色调、浅景深、缓慢推进”等正向前缀。已有语义和视觉签名只是软避重参考，不能迫使模型使用与产品无关的场景。抽象配方、成分、工艺、认证和背书只能作为商业背景，不能被写成肉眼已经证明的纹理、光泽、纯度或功效。

独立评估模板为 `effect-prompt-v11-creative-evaluation-v10`，评估模型会接收候选实际引用事实对应的同一份视觉策略，检查抽象事实是否被错误包装成视觉证明。商品无关、事实捏造和不可执行画面仍是硬问题；通用质感词堆叠、目的句代替可见动作和普通相似只使用稳定问题码记录软提醒。Worker 会确定性补充这些软提醒，但不会因此淘汰候选；回退新颖度计算覆盖叙事、场景、人物、产品关联点、镜头和情绪全部六维。

为降低单次创意调用的判断负担，Worker 会在分片前把原“主要事实”拆成两个角色：`visualTask` 必须成为本条可见画面，`businessContext` 只影响叙事和商业方向、不得承担视觉证明。生成模型不接收整张提炼信息表或整张策略表；每个 slot 的真实事实 ID 会进一步投影成该 slot 私有的 `F1/F2…` 局部别名，模型响应再由 Provider 按 slot 映射回真实事实，因此同一分片中的任务无法借用其他任务事实。返回结果仍必须使用本条可见任务和至少一个产品锚点。上游事实原文不会被改写，AI 只决定事实怎样安全进入画面。

批量生成采用以下规则：

- `BATCH_GENERATE` 会先用一次轻量规划形成 8～12 个批次创意方向。每个方向只引用已确认事实，包含恰好两个优先变化维度、六轴业务语义族和最多两个避让套路；检查点只在同一 Run 且上游、视觉策略、共用提示词、数量和模板哈希完全一致时复用。
- 上游 `visualStyleBaseline` 只在批次创意方向规划中作为共享视觉底色使用，模型需将其转译为光线、色彩、材质、镜头质感和情绪氛围；它不会进入单条创意任务或质量评估，也不得把整批固化为同一场景、人物、动作或镜头模板。
- `MMR_CONTENT_CLUSTER_V3` 首轮候选数为用户目标数量的 `140%`（向上取整），50 条目标生成 70 条初始候选。
...
- 新 Run 使用 `MMR_CONTENT_CLUSTER_V3`：按 `70%` 独立质量分和 `30%` 相对已选集合的新颖度逐条选择；新颖度内部由 `70%` 干净正文向量与 `30%` 六轴业务语义簇组成。
- 评估器只允许从本批动态词表或 `OTHER` 选择叙事、场景、人物关系、产品动作、镜头和情绪族；方向没有完全实现只记软提醒，事实虚构、商品无关和结构破损仍是硬问题。
...
- 最大场景族或产品动作族超过 40% 且存在兼容替代方向时，也可以触发同一次多样性补充；50 条目标最多补 10 条，总候选不超过 80 条。
...
当前实现接受 4～30 秒的统一目标时长。Worker 不把数字时长写入候选结果，而是为每个 `CreativeTask` 编译 `SHORT_FOCUS / COMPLETE_ACTION / GRADUAL_PROCESS / CONNECTED_PHASES` 四档软时序意图。评估输入按 `slotId` 带入权威时长，只通过现有 `visualExecutability` 和 `DURATION_TOO_DENSE / DURATION_TOO_SPARSE` 软提醒表达匹配度，不增加新评估字段或模型调用。分片恢复时从已持久化的 `creativePlan` 重建 `slotId → targetDurationSeconds`，不依赖模型回传。

API 只向 Worker 下发当前拓扑。旧拓扑任务在认领时会被安全终止，不会进入模型调用或被套用当前图继续执行；旧数据库记录只作为不可见审计数据保留。
...
队列信封兼容历史 Prompt 结果，新任务只使用当前批次契约。Ark 偶尔会把被截断的 JSON 标成 `completed`；当前实现对这种已完成但格式异常的响应仅在当前创意或评估分片内补试一次。补试仍失败时会取消同批尚未完成的调用，再交给任务层处理，避免旧 attempt 继续产生费用或回写。

批次去同质化同时检查正文向量、六轴业务语义、正文中的具体动作原子和底层场景原子。家庭厨房灶台、岭南厨房、砂锅台面和备餐区统一归入“厨房/备餐”，不能再靠不同模型标签伪造场景差异；方向分配、MMR 和定向补充都使用同一原子口径。只要两条候选共同使用“切片、拆袋、摆盘”等底层动作，也会降低新颖度。超过 40% 的底层场景或动作会触发一次定向补充，并把拥挤原子作为明确避让项传给补充任务。

多样性补充不是自动入选资格。初始池已能选满时，每个补充候选只有让当前选集的高风险近似对严格减少，或在近似对不增加时减少冗余候选，才允许参与替换；否则继续保留补充前的准确数量，避免“补充前 93 对、补充后 96 对”的反向优化。

对于不超过 30 秒的素材，Worker 会拒绝正文中的分钟级等待、从生制品完整熟制后继续切配上桌，以及“整根产品入沸水/蒸锅后片刻即成品”等无法实时拍完的过程；需要熟制的食品若直接铺到已完成饭菜或入口也会被拒绝。无工具或容器支撑的产品自转属于物理硬问题；筷子夹整根长条食品只记不自然操作提醒，避免误伤数量。抽象卖点的视觉证明由事实视觉策略与确定性规则共同判断，避免仅依赖模型评分。
...
- `ARK_PROMPT_STRATEGY_TIMEOUT_SECONDS`，默认 `180`；用于批次级创意方向规划，不影响候选生成超时
