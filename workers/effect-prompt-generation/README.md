...
当前方向模板在既有 `creativeDirection` 中引导 AI 说明观众关注点、已确认产品价值及承载它的可见事件；补充方向沿用相同原则。拍摄模型仍在一次调用中生成创意主线、六维和 `shotPlan`，编译器只分配时间和排版。镜头可通过注意力转移、细节揭示和人物选择推进信息，同一连续事件可合理切镜，也可一镜到底；不强制每条具有痛点、反转、口播和购买号召。

少量跨品类示例只示范表达方式，不作为当前商品事实或批次固定脚本。表演、逐字台词、现场声和焦点按需要写入已有字段，不增加输出字段、模型调用、质量门禁或 Worker 语义判断。时长软提示、事实边界、评估及去重算法保持不变。自动回归只验证模板接入与结构兼容；真实质量、重复度和耗时尚需同参数付费批次对比，不能由 Mock 结果推断提升。
