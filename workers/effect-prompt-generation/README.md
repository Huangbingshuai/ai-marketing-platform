...
随后批次创意导演根据可用事实、事实视觉策略、共用提示词和信息卡中的视觉风格基调动态规划 8～16 个创意方向；目标 50 条时规划 13 个方向，避免固定 8 个方向被批量改写成近义画面。风格基调只转译为光线、色彩、材质、镜头质感和情绪底色，不能反向虚构场景、人物或商品事实。每个方向由 AI 同时规划 2～4 项彼此有关联的业务事实，并逐项说明它们怎样进入人物、场景、需求、动作或产品表达；所有卖点、痛点、受众、决策动机、营销目标和已确认场景必须在整批方向中获得合理用途。每条创意完整继承所属方向的事实组合和 `productSnapshot`，模型仍在一次创意调用中同时生成：
