...
随后批次创意导演根据可用事实、事实视觉策略、共用提示词和信息卡中的视觉风格基调动态规划 8～16 个创意方向；目标 50 条时规划 13 个方向，避免固定 8 个方向被批量改写成近义画面。风格基调只转译为光线、色彩、材质、镜头质感和情绪底色，不能反向虚构场景、人物或商品事实。每条创意只接收一个必须实现的业务重点 `focusFact`、少量 `allowedFacts`、不参与重点竞争的 `productSnapshot`、重点事实自己的视觉策略和一个创意方向。模型仍在一次创意调用中同时生成：
...
独立评估模型会同时接收候选声明的事实和本条 `focusFact`，并复用同一份视觉策略检查抽象事实是否被错误包装成视觉证明。生成模型必须返回重点事实及一段来自正文、创意主线或六维字段的逐字证据；Worker 只验证事实引用和证据位置，评估模型再判断它是完整实现、部分实现还是未实现。只有语义完整且证据真实存在的事实才能形成绑定。包装规格不能证明配方，普通厨房不能证明传统工艺。商品无关、事实捏造和不可执行画面仍是硬问题；重点事实未完整实现会显著降低择优顺序，但不会让数量恢复流程直接失去候选。

为降低单次创意调用的判断负担，Worker 不再把事实拆成视觉任务、业务背景、产品锚点和产品边界等竞争角色。批次调度先为每条任务选择一个卖点、痛点、受众、决策动机、营销目标或场景作为 `focusFact`；产品名称、品类、规格和外观由 `productSnapshot` 统一提供真实性边界，不能替换业务重点。生成模型不接收整张提炼信息表或整张策略表，也不能跨任务引用其他事实。上游事实原文不会被改写，AI 只决定重点事实怎样安全进入同一条连贯画面。
...
- 当前连贯创意简报只有一个 `focusFact`。`allowedFacts` 提供最多数条自然搭配信息，`productSnapshot` 保存产品名称、品类、规格和视觉特征，只用于保持身份与实物边界。商品快照可以帮助正文明确产品，但不能取代卖点、痛点、受众或场景成为主要业务事实。
...
队列信封只携带当前任务标识，不携带 Prompt 代码版本。Ark 偶尔会把被截断的 JSON 标成 `completed`；候选生成的事实编号会先转换为每条任务私有的 `F1/F2...`，模型返回后再映射回权威事实 ID，避免同一次请求中的多条任务串用事实。新候选必须原样返回本条 `focusFactId`，把它加入声明事实，并给出确实存在于正文、创意主线或六维字段中的逐字证据；缺少重点、引用越界或证据位置不存在时只隔离该候选。产品身份是否充分和重点事实是否语义完整由独立评估模型判断。批量生成分片连续两次无有效候选时会跳过该分片并交给现有数量补充流程，不降低事实安全门槛，也不再让单条异常拖垮整批。
