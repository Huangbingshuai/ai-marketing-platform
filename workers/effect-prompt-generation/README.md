...
随后先由 AI 根据可用事实、事实视觉策略、共用提示词和视觉风格基调规划产品专属创意版图，再由创意导演按版图动态规划 8～16 个创意方向；目标 50 条时规划 13 个方向。版图中的场景空间、主动作和建议承载数都由模型按当前商品产生，不在 Worker 中维护“厨房、切片”等商品关键词。版图还必须逐项说明每个兼容事实的自然用法和不成立条件；方向完成后，独立 AI 不仅重新判断实际版图和主动作，还逐项复核方向中的事实关系是否自然、是否依赖未确认画面条件、是否把抽象事实伪装成视觉证明。发现问题时把诊断交回方向模型重规划被点名方向。Worker 只校验事实来源、稳定 ID、数量、哈希和复核覆盖，不判断业务语义，也不随机补挂事实。
...
事实视觉策略、产品创意版图和创意方向在单次模型请求内使用 `F1/F2...` 短事实别名，返回后再机械恢复为权威事实 ID，避免模型抄错长哈希。创意版图或方向若返回完整但破损的 JSON，只局部重试当前子步骤一次；事实主分配或配额无效时由 AI 整体重规划一次，Worker 不随机补挂事实、不修改场景语义。版图的 `requiredFactIds` 必须同时存在于 `compatibleFactIds`，并拥有对应的 `factCompatibilities` 自然用法说明；Worker只核对这些集合是否完整一致，不再替模型把缺失事实机械并入兼容清单。
