...
随后先由 AI 根据可用事实、事实视觉策略、共用提示词和视觉风格基调规划产品专属创意空间；第二次紧凑 AI 调用再把每项必用业务事实唯一分配给一个自然相容空间；最后由创意导演按通过复核的版图动态规划 8～16 个创意方向。方向数同时受目标 Prompt 数和必用业务事实数影响，目标 50 条且 37 项事实时规划 13 个方向。空间和事实的业务关系都由模型判断；同一自然主题可以承载较多事实，Worker 按每方向最多 4 项事实计算该空间需要的方向槽位，只检查事实覆盖、空间 ID、数量、结构和总容量。Worker 不维护“厨房、切片”等商品关键词，也不把遗漏事实随机塞进某个空间。

当前实现已用已提交的广式腊肠完整信息表完成受控真实 Ark 复验，并停在单条 Prompt 生成之前：6 个真实创意空间、13 个方向、37/37 项强制业务事实全部覆盖，形成 13 组不同事实组合、13 组不同六轴语义档案和 12 组场景主动作组合，独立语义复核最终无遗留问题。该结果只证明方向源头和事实分配链路有效，不等同于最终 50 条 Prompt 质量验收；完整数据和人工风险复核见对应实施方案。
