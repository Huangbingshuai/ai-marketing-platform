...
随后先由 AI 根据可用事实、事实视觉策略、共用提示词和视觉风格基调规划产品专属创意空间；第二次紧凑 AI 调用再把每项必用业务事实唯一分配给一个自然相容空间；最后由创意导演按通过复核的版图动态规划 8～16 个创意方向，目标 50 条时规划 13 个方向。空间和事实的业务关系都由模型判断，Worker 只检查事实是否恰好覆盖一次、空间 ID 是否存在、每个空间是否不超过 8 项事实，以及按“每方向最多 4 项事实”计算的容量是否足够。Worker 不维护“厨房、切片”等商品关键词，也不把遗漏事实随机塞进某个空间。

当前实现已用已提交的广式腊肠完整信息表完成一次受控真实 Ark 复验，并停在单条 Prompt 生成之前：9 个创意空间、13 个方向、38/38 项强制业务事实全部获得唯一主承载，13 组事实组合和场景动作组合均不重复，独立语义复核一次通过。该结果只证明方向源头和事实分配链路有效，不等同于最终 50 条 Prompt 质量验收；完整数据和人工风险复核见对应实施方案。

风格基调只转译为光线、色彩、材质、镜头质感和情绪底色，不能反向虚构场景、人物或商品事实。每个方向由 AI 同时规划 2～4 项彼此有关联的业务事实，并逐项说明它们怎样进入人物、场景、需求、动作或产品表达；所有卖点、痛点、受众、决策动机、营销目标和已确认场景必须在整批方向中获得合理用途。每条创意完整继承所属方向的事实组合和 `productSnapshot`，模型仍在一次创意调用中同时生成：
...
- 当前连贯创意简报使用方向模型给出的 `factApplications`，每项包含原始事实和该事实在当前创意中的具体用途。富信息商品的每个方向必须包含 2～4 项业务事实；资料本身只有一个业务事实时才允许单事实方向。`productSnapshot` 保存产品名称、品类、规格和视觉特征，只用于保持身份与实物边界。
...
事实视觉策略、产品创意空间、事实主承载分配和创意方向在模型请求内使用 `F1/F2...` 短事实别名，返回后再机械恢复为权威事实 ID。创意空间 JSON 格式损坏时只局部重取，不占用业务语义修订预算；事实分配缺失、重复、未知引用、单空间超过 8 项或方向容量不足时，只重取紧凑的事实分配，不重写整套空间。只有独立 AI 认为事实与空间语义不自然时，才重新规划产品创意空间。Worker不修改场景语义，也不自行决定事实应去哪个空间。
