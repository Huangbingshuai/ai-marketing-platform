...
图片分析先提取可见外观、包装文字、陈列、场景与视觉气氛，也可以在画面具有明确人物、动作、用途或购买情境时，保守补充卖点、痛点、受众、决策动因、营销目标和场景建议。节庆、礼赠、家庭餐桌等画面证据优先进入对应场景字段；香料、竹篮、蒸笼、餐具和布景不得作为产品卖点。价格、配方、产地、认证、功效、销量与信任背书仍不得无证据推断。卖点融合先收集全部用户事实和有证据的图片建议，再由语义模型在最终容量内择优；AI 生成容量统一为核心卖点最多 3 项、次要卖点最多 6 项、辅助信任背书/受众/痛点/决策动因/三类场景最多 5 项，用户手工编辑仍使用 20 项公共安全边界。用户次要卖点不超过 6 项时必须全部保留，超过 6 项时模型只能在用户事实中择优。核心卖点、次要卖点、痛点、决策动因以及三类场景统一交给语义模型判断关系，NORMALIZATION 只复用模型整理后的候选并恢复用户权威来源；核心外观特征只有在用户资料没有提供时才使用图片结果。
