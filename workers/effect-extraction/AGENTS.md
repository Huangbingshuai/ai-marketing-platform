...
`LOAD_AND_SNAPSHOT → DOCUMENT / IMAGE / COMMERCE / FORM → FUSION → NORMALIZATION`
...
- 输出遵循 Result V2《产品素材制作信息卡》五层：产品基础层、卖点层、用户层、场景层、制作规则层。事实、推断、建议、警告、来源引用与质量摘要不可混在一个自由文本字段。
...
- 重新提炼继承人工覆盖，并在 Worker 完成后由 API 确定性再次应用；历史 V1 通过统一适配器读取，不调用模型迁移，也不隐式提交 V2 revision。
